Shader "DuneVector/URP TV Static"
{
    Properties
    {
        ...
        _FrameRate("Frame Rate (0 is Continuous)", Float) = 24
        _TimeScale("Time Scale", Float) = 1
        _Seed("Seed", Float) = 0
        _NoiseContrast("Noise Contrast", Range(0, 8)) = 1.35
        _NoiseBrightness("Noise Brightness", Range(-1, 1)) = 0
        _ColorNoise("Color Noise", Range(0, 1)) = 0
        [HDR] _StaticDarkColor("Static Dark Color", Color) = (0.015, 0.015, 0.02, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Frag(Varyings input) : SV_Target
            {
                ...
                float staticBlend = saturate(_StaticAmount + (rollBar * _RollBarStaticBoost));
                ...
                float vignetteFalloff = pow(
                    saturate(length(centered) * _VignetteScale),
                    max(0.1, _VignettePower));
                float vignette = 1.0 - (saturate(vignetteFalloff) * saturate(_VignetteStrength));
                color *= vignette;
